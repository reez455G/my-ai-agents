...
**Koreksi (2026-07-19):** paragraf di atas sudah **superseded** — `setup-new-device.sh` diperluas untuk meng-cover Syncthing: install (`apt`) + `systemctl --user enable --now syncthing` kalau belum aktif, print Device ID device ini, alias `omp`/`omp-sync` versi baru (step "5b" ditulis ulang, bukan lagi alias git-based), dan step baru "5c" yang opsional-interaktif — kalau operator sudah punya Device ID device lawan, script langsung menjalankan pairing REST API (add device + share folder `omp-skills`) di sisi lokal. Sisi device lawan tetap harus menjalankan pairing balik secara manual/lewat run script itu sendiri di sana — satu invocation hanya bisa mengatur config Syncthing lokal.

**Tiga bug ditemukan & diperbaiki saat onboarding device ke-3 (2026-07-19), semua ditemukan lewat kejadian nyata bukan review kode statis:**

1. **Path folder salah kalau `$HOME` ≠ lokasi repo.** Troubleshooting manual di device ke-3 (run sebagai `root`, repo di-clone ke `/opt/my-ai-agents`) sempat pakai `$HOME/my-ai-agents/.omp/skills` sebagai path — karena `$HOME` untuk `root` adalah `/root`, Syncthing bikin folder baru KOSONG di `/root/my-ai-agents/.omp/skills`, terpisah total dari repo asli. Fix: `setup-new-device.sh` §5c dan README pakai `$PWD`/`$(pwd)` (lokasi repo saat script/perintah dijalankan), bukan asumsi `$HOME`. Sudah diverifikasi via self-check di script: bandingkan `path` hasil `GET /rest/config/folders/omp-skills` dengan `$PWD/.omp/skills`, warn keras kalau tidak cocok alih-alih diam.

2. **`systemctl --user enable --now syncthing` bisa silent-fail** di device tanpa user session aktif (umum di `root`/VPS/container — device ke-3 kasusnya persis ini). Script sebelumnya cuma `sleep 2` lalu langsung coba pairing REST API tanpa mengecek daemon benar-benar hidup, jadi kegagalan pairing waktu itu tidak jelas sebabnya (API key ok, tapi daemon belum listen). Fix: tambah fallback `systemctl enable --now syncthing@$(id -un)` (unit template per-user, jalan tanpa session bus), dan polling nyata ke REST API (bukan asumsi dari exit code systemctl) sebelum lanjut ke langkah pairing — kalau gagal, bilang eksplisit "daemon belum reachable", bukan diam-diam lanjut dengan asumsi salah.

3. **`/rest/noauth/health` mengembalikan 403 CSRF Error** di build Syncthing Debian-patched (`v1.19.2-ds1`) yang dipakai beberapa device di sini — endpoint unauthenticated ini ternyata tidak konsisten antar-build/versi. Diganti pakai `/rest/system/ping` (authenticated, `X-API-Key`), yang terverifikasi jalan di semua device yang sudah dicek.

4. **PUT ke `/rest/config/folders/<id>` REPLACE seluruh `devices` list folder itu, bukan menambah.** Ditemukan pas testing ulang fix di atas: re-pairing ke device 2 (yang sudah lama tersambung) lewat script yang mengirim `"devices": [{"deviceID": peer_id}]` diam-diam **menghapus device 3** dari folder `omp-skills` (folder cuma menyisakan device yang barusan di-PUT). Ini paling berbahaya dari empat bug karena tidak ada error/warning — sync-nya tetap kelihatan "berhasil", cuma device lain kehilangan akses tanpa pemberitahuan. Fix: baca `devices` existing dari folder (`GET` dulu), gabung dengan peer baru, baru `PUT` — di `setup-new-device.sh` §5c dan contoh REST API manual di README. Prinsip: PUT ke path spesifik sebuah *item* (`/devices/<id>`) aman (upsert satu entry), tapi PUT ke *field koleksi* (`devices` di dalam objek folder) selalu perlu read-modify-write, tidak boleh dianggap upsert.

**Pelajaran umum:** ketiganya lolos dari code review karena kelihatan benar secara sintaks dan bahkan berhasil di test pertama (device 2, LAN, user biasa, folder baru) — baru ketahuan saat kondisi berbeda (root user, path custom, network beda, folder yang SUDAH punya history). Verifikasi ulang tiap fix lewat eksekusi nyata (bukan cuma `bash -n`) sebelum dianggap selesai — persis yang membongkar bug #4 di atas.
