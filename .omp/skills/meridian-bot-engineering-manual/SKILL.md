---
name: meridian-bot-engineering-manual
description: CLAUDE.md engineering manual for the Meridian Solana DLMM trading bot repo — full module map, SCREENER/MANAGER/GENERAL agent roles, the ReAct loop, cron/cycle architecture, position lifecycle, persistent JSON state files, and known tech debt. Use when making non-trivial code changes inside the meridian repo (new tool, safety rule, cron logic, state file).
---
